...
def _key(board):
    ...
    return (board.pawns, board.knights, board.bishops, board.rooks,
            board.queens, board.kings, board.occupied_co[0],
            board.occupied_co[1], board.turn, board.clean_castling_rights(),
...
def _allocate(time_left_ms, increment_ms=500):
    ...
    hard = min(5.0, inc * 0.76 + max(0.0, remaining - 2.0) / 42.0)
    hard = min(hard, usable * 0.22)
    return hard * 0.62, hard
    ...
    def evaluate(self):
        ...
        for color, occ, own, enemy, sign in ((1, white, wp, bp, 1), (0, black, bp, wp, -1)):
            ...
            for sq in scan(own):
                ...
                if not (PASSED[color][sq] & enemy):
                    if not (FORWARD[color][sq] & own):
                        mg += sign * PASS_MG[rank]
                        eg += sign * PASS_EG[rank]
                        ahead = sq + (8 if color else -8)
                        if 0 <= ahead < 64 and not (BB[ahead] & b.occupied):
                            eg += sign * (PASS_EG[rank] // 4)
                neighbours = (FILES[file - 1] if file else 0) | (FILES[file + 1] if file < 7 else 0)
                if not (neighbours & own):
                    mg -= sign * 10
                    eg -= sign * 8
