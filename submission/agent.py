...
def _has_legal_move(board):
    """Prove existence with a pawn push, or generate moves. Requires no check."""
    pawns = board.pawns & board.occupied_co[board.turn]
    if pawns:
        blocked = board.occupied >> 8 if board.turn else board.occupied << 8
        pushers = pawns & ~blocked & ~RANKS[7 if board.turn else 0]
        if pushers:
            square = (pushers & -pushers).bit_length() - 1
...
def _see(board, move, threshold=None):
    """Static exchange value with x-rays and legal, unpinned recaptures.

    This original implementation applies the standard least-valuable-attacker
    exchange algorithm. Both sides can stop exchanging at any point.
    """
    ...
    if victim is None:
        if board.is_en_passant(move):
            ...
            victim = 0
    gains = [CAP_VALUE[victim] + (CAP_VALUE[move.promotion] - 100 if move.promotion else 0)]
    if moving == chess.KING:
        return gains[0]
    current_value = CAP_VALUE[move.promotion or moving]
    side = not board.turn
    ...
    while True:
        ...
        if not chosen:
            break
        if threshold is not None and len(gains) == 1 and target >> 3 not in (0, 7):
            # After this legal reply we can gain at most the recapturer's
            # value: the opponent may decline every subsequent exchange.
            upper_bound = gains[0] - current_value + CAP_VALUE[piece]
            if upper_bound < threshold:
                return upper_bound
        promotion = piece == chess.PAWN and target >> 3 in (0, 7)
        gains.append(current_value - gains[-1] + (875 if promotion else 0))
        current_value = CAP_VALUE[chess.QUEEN if promotion else piece]
        occupied ^= chosen
        side = not side
        if piece == chess.KING:
            break
...
class _Game:
    ...
    def retain(self, board, move):
        ...
        rights_before = board.clean_castling_rights()
        self.seen = [] if self.our_zeroing else self.root_history[:]
        board.push(move)
        rights = board.clean_castling_rights()
        if rights != rights_before:
            self.seen.clear()
        self.last_rights = rights
...
class _Search:
    ...
    def ordered_moves(self, ttmove, ply, check):
        """Generate later stages only if earlier moves do not cause a cutoff."""
        b = self.b
        if check:
            ...
            yield from moves
            return
        if ttmove is not None:
            # The exact position key guarantees a stored legal move is legal.
            yield ttmove
        captures = list(b.generate_legal_captures())
        promotion_rank = RANKS[6 if b.turn else 1]
        if b.pawns & b.occupied_co[b.turn] & promotion_rank:
            captures.extend(m for m in b.generate_legal_moves(from_mask=promotion_rank & b.pawns, to_mask=~b.occupied) if m.promotion)
        captures.sort(key=lambda m: self.move_score(m, None, ply), reverse=True)
        for move in captures:
            if move != ttmove:
                yield move
        killers = self.killers[ply]
        for killer in killers:
            if killer is not None and killer != ttmove and not b.is_capture(killer) and b.is_legal(killer):
                yield killer
        quiets = [move for move in b.generate_legal_moves(to_mask=~b.occupied_co[not b.turn])
                  if not move.promotion and move != ttmove and move not in killers
                  and not (move.to_square == b.ep_square and b.pawns & BB[move.from_square])]
        history = self.history[b.turn]
        quiets.sort(key=lambda m: history[m.from_square * 64 + m.to_square], reverse=True)
        yield from quiets
        ...
        try:
            for move in moves:
                ...
                if not check and not move.promotion and best > -MATE_BOUND:
                    ...
                    attacker = b.piece_type_at(move.from_square)
                    prune = best + CAP_VALUE[victim] + 240 < alpha
                    if not prune and qdepth >= 2 and CAP_VALUE[attacker] > CAP_VALUE[victim]:
                        prune = _see(b, move, -60) < -60
                self.play(move)
                try:
                    # Preserve checking sacrifices and all evasions/promotions.
                    if prune and not b.is_check():
                        continue
    ...
    def search(self, depth, alpha, beta, ply, allow_null=True, null_tree=False):
        ...
        if (allow_null and not null_tree and not pv and not check and depth >= 3
                ...
                and b.halfmove_clock < 90 and _has_legal_move(b)):
        ...
        moves = self.ordered_moves(ttmove, ply, check)
        mg, eg, phase = self.mg, self.eg, self.phase
